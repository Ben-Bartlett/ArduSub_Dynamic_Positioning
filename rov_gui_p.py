...
def cancel_command():
    ...
    while reading_gotten == False:
        ...
        if 'LOCAL_POSITION_NED' in data:
            current_x = data['LOCAL_POSITION_NED']['x']
            current_y = data['LOCAL_POSITION_NED']['y']
            current_z = data['LOCAL_POSITION_NED']['z']
            current_vx = data['LOCAL_POSITION_NED']['vx']
            current_vy = data['LOCAL_POSITION_NED']['vy']
            current_vz = data['LOCAL_POSITION_NED']['vz']
            reading_gotten =  True
	    print(current_x,current_y,current_z,current_vx,current_vy,current_vz)
    ...
    master.mav.set_position_target_local_ned_send(
        ...
        mavutil.mavlink.MAV_FRAME_LOCAL_OFFSET_NED, #MAV_FRAME_BODY_OFFSET_NED, # offset to current position and heading, MAV_FRAME_LOCAL_OFFSET_NED for testing recording as it only gives position in NED
        0b110111000000, # 
        float(current_x), float(current_y), float(current_z), # x y z 
        float(current_vx), float(current_vy), float(current_vz), #vx vy vz
        0, 0, 0, # afx afy afz
        0, 0) # yaw # yawrate
...
def set_pitch(down_up):
    ...
    while reading_gotten == False:
        ...
        data = update()
        if 'ATTITUDE' in data:
            current_roll = data['ATTITUDE']['roll']
            current_pitch = data['ATTITUDE']['pitch']
            current_heading = data['ATTITUDE']['yaw']
            #print(current_roll, current_pitch, current_heading)
	    reading_gotten =  True


    # catch letters etc
    try:
	#print(txt_set_pitch.get())
	if abs(float(txt_set_pitch.get())) < 45:
            pitch = down_up * abs(float(txt_set_pitch.get()))
            ...
            master.mav.set_attitude_target_send(
                ...
                0, 0,   
                bitmask,
                QuaternionBase([float(current_roll), math.radians(float(pitch)), float(current_heading)]), # -> attitude quaternion (w, x, y, z | zero-rotation is 1, 0, 0, 0)
                0, #roll rate
                0, #pitch rate
                0, 0)    # yaw rate, thrust 
...
def set_roll(left_right):
    ...
    # catch letters etc
    try:
	#print(txt_set_roll.get())
	if abs(float(txt_set_roll.get())) < 45:
            roll = left_right * abs(float(txt_set_roll.get()))
            ...
            master.mav.set_attitude_target_send(
                ...
                0, 0,   
                bitmask,
                QuaternionBase([math.radians(float(roll)), float(current_pitch), float(current_heading)]), # -> attitude quaternion (w, x, y, z | zero-rotation is 1, 0, 0, 0)
                0, #roll rate
                0, #pitch rate
                0, 0)    # yaw rate, thrust 
...
def set_velocity_xyz(xyz, lr_fb):
	...
	bitmask=0b110111000000
	
        if xyz == 'x':
            if abs(float(txt_set_vxy.get())) < 10:
                vx = lr_fb*abs(float(txt_set_vxy.get()))
		print(vx)
                ...
                0, 0, # target system_id # target component id
                mavutil.mavlink.MAV_FRAME_BODY_OFFSET_NED, #MAV_FRAME_BODY_OFFSET_NED, # offset to current position and heading, MAV_FRAME_LOCAL_OFFSET_NED for testing recording as it only gives position in NED
                bitmask, # mask specifying use-only-x-y-z
                0, 0, 0, # x y z 
                vx, 0, 0, #vx vy vz
                0, 0, 0, # afx afy afz
                0, 0) # yaw # yawrate
            ...
            if abs(float(txt_set_vxy.get())) < 10:
		vy = lr_fb*abs(float(txt_set_vxy.get()))
            ...
            if abs(float(txt_set_vz.get())) < 10:
		vz = lr_fb*abs(float(txt_set_vz.get()))
...
def pos_to_vel(flag):
    ...
    if flag == -1:
        button_pos_mode.grid_remove()
	label_depth.grid_remove()
	txt_set_depth.grid_remove()
	button_set_depth_down.grid_remove()
	button_set_depth_up.grid_remove()
	frame_distance.grid_remove()
	button_set_x_forward.grid_remove()
	button_set_x_back.grid_remove()
	button_set_y_left.grid_remove()
	button_set_y_right.grid_remove()
	frame_x.grid_remove()
	frame_y.grid_remove()

        button_vel_mode.grid(column=6, row=9, pady=10)
        label_vz.grid(column=0, row=1)
	txt_set_vz.grid(column=1, row=1)
	button_set_vz_pos.grid(column=2, row=1)
	button_set_vz_neg.grid(column=3, row=1)
	frame_velocity.grid(column=2, row=11)
	button_set_vx_forward.grid(column=2, row=9, pady=10)
	button_set_vx_back.grid(column=2, row=12, pady=10)
	button_set_vy_left.grid(column=1, row=11)
	button_set_vy_right.grid(column=3, row=11)
	frame_vx.grid(column=2, row=8, padx=15)
	frame_vy.grid(column=0, row=11, padx=(40,0))
	

        pos_to_vel.pv = 'p'

    if flag == 1:
        button_vel_mode.grid_remove()
	label_vz.grid_remove()
	txt_set_vz.grid_remove()
	button_set_vz_pos.grid_remove()
	button_set_vz_neg.grid_remove()
	frame_velocity.grid_remove()
	button_set_vx_forward.grid_remove()
	button_set_vx_back.grid_remove()
	button_set_vy_left.grid_remove()
	button_set_vy_right.grid_remove()
	frame_vx.grid_remove()
	frame_vy.grid_remove()
	
        button_pos_mode.grid(column=6, row=9, pady=10)
...
frame_p = Frame(window) 
frame_p.grid(column=0, row=14, padx=(40,0), pady=(60,50))

label_p = Label(frame_p, text="PITCH")
label_p['font'] = font.Font(size=15)
label_p.pack(side=TOP)

image_p_down = Image.open("gui_pics/pitch_down.png")
image_p_down = image_p_down.resize((50,50),Image.ANTIALIAS) 
photoImg_p_down = ImageTk.PhotoImage(image_p_down)

image_p_up = Image.open("gui_pics/pitch_up.png")
image_p_up = image_p_up.resize((50,50),Image.ANTIALIAS) 
photoImg_p_up = ImageTk.PhotoImage(image_p_up)

txt_set_pitch = Entry(window,width=10)
txt_set_pitch.grid(column=1, row=14)


button_set_p_down = Button(window, text="SET", command=lambda: set_pitch(-1), image=photoImg_p_down, compound=LEFT) 
button_set_p_down.grid(column=2, row=14)

button_set_p_up = Button(window, text="SET", command=lambda: set_pitch(1), image=photoImg_p_up, compound=LEFT) 
button_set_p_up.grid(column=3, row=14)

#roll addition

frame_r = Frame(window) 
frame_r.grid(column=0, row=15, padx=(40,0))

label_r = Label(frame_r, text="ROLL")
label_r['font'] = font.Font(size=15)
label_r.pack(side=TOP)

image_r_left = Image.open("gui_pics/roll_left.png")
image_r_left = image_r_left.resize((50,50),Image.ANTIALIAS) 
photoImg_r_left = ImageTk.PhotoImage(image_r_left)

image_r_right = Image.open("gui_pics/roll_right.png")
image_r_right = image_r_right.resize((50,50),Image.ANTIALIAS) 
photoImg_r_right = ImageTk.PhotoImage(image_r_right)

txt_set_roll = Entry(window,width=10)
txt_set_roll.grid(column=1, row=15)


button_set_r_left = Button(window, text="SET", command=lambda: set_roll(1), image=photoImg_r_left, compound=LEFT) 
button_set_r_left.grid(column=2, row=15)

button_set_r_right = Button(window, text="SET", command=lambda: set_roll(-1), image=photoImg_r_right, compound=LEFT) 
button_set_r_right.grid(column=3, row=15)
...
#####################################################################
image_cancel = Image.open("gui_pics/cancel.png")
image_cancel = image_cancel.resize((45,45),Image.ANTIALIAS) 
photoImg_cancel = ImageTk.PhotoImage(image_cancel)

button_cancel = Button(window, text="CANCEL", command=lambda: cancel_command(), image=photoImg_cancel, compound=LEFT) 
button_cancel['font'] = font.Font(size=20)
button_cancel.grid(column=0, row=4, pady=10, padx=5)


# velocity instead of position 
image_position = Image.open("gui_pics/position.png")
image_position = image_position.resize((45,45),Image.ANTIALIAS) 
photoImg_position = ImageTk.PhotoImage(image_position)

button_pos_mode = Button(window, text="POSITION MODE", command=lambda: pos_to_vel(-1), image=photoImg_position, compound=LEFT) 
button_pos_mode['font'] = font.Font(size=8)
button_pos_mode.grid(column=6, row=9, pady=10)

image_velocity = Image.open("gui_pics/velocity.png")
image_velocity = image_velocity.resize((43,45),Image.ANTIALIAS) 
photoImg_velocity = ImageTk.PhotoImage(image_velocity)


button_vel_mode = Button(window, text="VELOCITY MODE", command=lambda: pos_to_vel(1), image=photoImg_velocity, compound=LEFT) 
button_vel_mode['font'] = font.Font(size=8)
#button_vel_mode.grid(column=6, row=9, pady=10)

##############################################################################
#vel buttons
label_vz = Label(window, text="Vertical Velocity")
label_vz['font'] = font.Font(size=15)
#label_vz.grid(column=0, row=1)

txt_set_vz = Entry(window,width=10)
#txt_set_vz.grid(column=1, row=1)

button_set_vz_pos = Button(window, text="SET", command=lambda: set_velocity_xyz('z',1), image=photoImg_depth_down, compound=LEFT) 
#button_set_vz.grid(column=2, row=1)

button_set_vz_neg = Button(window, text="SET", command=lambda: set_velocity_xyz('z',-1), image=photoImg_depth_up, compound=LEFT) 
#button_set_vz.grid(column=3, row=1)


# X, Foward/Back /Surge
# Y, Left/Right /Sway

frame_velocity = Frame(window) # to hold both txt box and txt in same row of window
#frame_velocity.grid(column=2, row=11)

label_velocity = Label(frame_velocity, text="Velocity (m/s)")
label_velocity.pack(side=TOP)

txt_set_vxy = Entry(frame_velocity,width=10)
txt_set_vxy.pack(side=BOTTOM, pady=5)

button_set_vx_forward = Button(window, text="SET", command=lambda: set_velocity_xyz('x',1), image=photoImg_x_forward, compound=LEFT) 
#button_set_vx_forward.grid(column=2, row=9, pady=10)

button_set_vx_back = Button(window, text="SET", command=lambda: set_velocity_xyz('x',-1), image=photoImg_x_back, compound=LEFT) 
#button_set_vx_back.grid(column=2, row=12, pady=10)

button_set_vy_left = Button(window, text="SET", command=lambda: set_velocity_xyz('y',-1), image=photoImg_y_left, compound=LEFT) 
#button_set_vy_left.grid(column=1, row=11)

button_set_vy_right = Button(window, text="SET", command=lambda: set_velocity_xyz('y',1), image=photoImg_y_right, compound=LEFT) 
#button_set_vy_right.grid(column=3, row=11)

frame_vx = Frame(window) 
#frame_vx.grid(column=2, row=8, padx=15)

label_vx = Label(frame_vx, text="SURGE")
label_vx['font'] = font.Font(size=15)
label_vx.pack(side=LEFT)
label_vx_frame = Label(frame_vx, text="forward")
label_vx_frame['font'] = font.Font(size=8)
label_vx_frame.pack(side=RIGHT)


frame_vy = Frame(window) 
#frame_vy.grid(column=0, row=11, padx=(40,0))

label_vy = Label(frame_vy, text="SWAY")
label_vy['font'] = font.Font(size=15)
label_vy.pack(side=TOP)
label_vy_frame = Label(frame_vy, text="left")
label_vy_frame['font'] = font.Font(size=8)
label_vy_frame.pack(side=BOTTOM)
