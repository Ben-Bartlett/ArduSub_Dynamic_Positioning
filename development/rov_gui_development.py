'''
   THIS FILE WAS THE FILE THAT WAS "FINISHED" FOR THE FINAL YEAR PROJECT BUT THE ROV_GUI.PY FILE
   IS THE CLEANED VERSION FOR STRAIGHT FORWARD USE OF THE PROGRAM
'''
...
window.rowconfigure(3, {'minsize': 30}) #spacer for XY control area
...
window.columnconfigure(2, {'minsize': 130})# ''
...
# CHANGE MODE
button_change_mode_to_dyna = Button(window, text="DYNAMIC POSITIONING\nFLIGHT MODE", command=change_mode_dyna) 
...
label_heading = Label(window, text="HEADING")
label_heading.grid(column=0, row=0)
...
frame_distance.grid(column=3, row=11)

label_distance = Label(frame_distance, text="DISTANCE")
...
button_set_x_forward.grid(column=3, row=9, pady=10)

button_set_x_back = Button(window, text="SET", command=lambda: set_xy('x',-1), image=photoImg_x_back, compound=LEFT) 
button_set_x_back.grid(column=3, row=12, pady=10)

button_set_y_left = Button(window, text="SET", command=lambda: set_xy('y',-1), image=photoImg_y_left, compound=LEFT) 
button_set_y_left.grid(column=2, row=11)

button_set_y_right = Button(window, text="SET", command=lambda: set_xy('y',1), image=photoImg_y_right, compound=LEFT) 
button_set_y_right.grid(column=4, row=11)

label_x = Label(window, text="SURGE")
label_x.grid(column=3, row=8, padx=15)

label_y = Label(window, text="SWAY")
label_y.grid(column=1, row=11, padx=(40,0))

# Record/ Stop
button_start_depth_record = Button(window, text="Record Depth\nStart", command=lambda: start_stop_read_depth(1))
button_start_depth_record.grid(column=2, row=2)

button_stop_depth_record = Button(window, text="Record Depth\nStop",  command=lambda: start_stop_read_depth(0)) 
button_stop_depth_record.grid(column=3, row=2)

button_start_heading_record = Button(window, text="Record Heading\nStart", command=lambda: start_stop_read_heading(1))
button_start_heading_record.grid(column=3, row=0)

button_stop_heading_record = Button(window, text="Record Heading\nStop",  command=lambda: start_stop_read_heading(0)) 
button_stop_heading_record.grid(column=4, row=0)

frame_record_start_stop_xy = Frame(window) # to hold both buttons in same column of window
frame_record_start_stop_xy.grid(column=6, row=11, padx=10)

button_start_xy_record = Button(frame_record_start_stop_xy, text="Record XY\nStart", command=lambda: start_stop_read_xy(1))
button_start_xy_record.pack(side=LEFT)

button_stop_xy_record = Button(frame_record_start_stop_xy, text="Record XY\nStop",  command=lambda: start_stop_read_xy(0)) 
button_stop_xy_record.pack(side=RIGHT)

# test routine
frame_test_routine = Frame(window) 
frame_test_routine.grid(column=6, row=12)

button_test_routine = Button(frame_test_routine, text="test routine",  command=test_routine)
button_test_routine.pack(side=TOP, pady=5)

frame_test_routine_sub = Frame(frame_test_routine) 
frame_test_routine_sub.pack(side=BOTTOM, pady=5)

txt_test_x = Entry(frame_test_routine_sub,width=3)
txt_test_x.pack(side=LEFT, padx=5)
txt_test_y = Entry(frame_test_routine_sub,width=3)
txt_test_y.pack(side=LEFT, padx=5)
txt_test_z = Entry(frame_test_routine_sub,width=3)
txt_test_z.pack(side=LEFT, padx=5)
txt_test_heading = Entry(frame_test_routine_sub,width=3)
txt_test_heading.pack(side=RIGHT, padx=5)
